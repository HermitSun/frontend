        ...
        <el-table :data="students" :highlight-current-row="true" v-loading="listLoading" ref="table"
            ...
            <el-table-column label="审核结果" width="150" :sortable="true" :sort-method="sortByResult">
                <template slot-scope="scope">
                    <el-tag type="info" v-if="scope.row.from===0">未处理</el-tag>
                    <el-tag type="success" v-else-if="scope.row.from===3">通过</el-tag>
                    <el-tag type="danger" v-else-if="scope.row.from===4">未通过</el-tag>
                </template>
            </el-table-column>
        ...
        methods: {
            ...
            getStudents() {
                ...
                checkStuList({
                    ...
                    checkStuList({
                        from: _this.froms.SENIOR_FAILED,
                        page: _this.page,
                    ...
                    checkStuList({
                        from: _this.froms.SENIOR_PASSED,
                        page: _this.page,
                        name: _this.filters.name
                    }).then((res) => {
                        _this.total += res.data.total;
            ...
            handlePass(row) {
                this.$confirm('确认修改？', '提示', {}).then(() => {
                    let params = {
                        ...
                        from: this.froms.SENIOR_PASSED
                    };
                    if (row.from === this.froms.SENIOR_PASSED) {
                        params.from = this.froms.SENIOR_FAILED;
                    }
